...
class FileProcessor(QThread):
    ...
    def __init__(self, files, drop_threshold, global_dd, time_distance):
        super().__init__()
        self.files = files
        self.drop_threshold = drop_threshold
        self.global_dd = global_dd
        self.time_distance = time_distance

    def run(self):
        # Initialize data structures
        all_csv_data = []  # List to hold all rows from all files

        total_files = len(self.files)
        for idx, csv_file in enumerate(self.files):
            self.log.emit(f"Processing file: {csv_file.name}")
            file_bytes = csv_file.read()
            detected_encoding = chardet.detect(file_bytes)['encoding']

            if detected_encoding is None:
                self.log.emit(f"Skipping {csv_file.name} due to undetected encoding.")
                continue

            try:
                decoded_file_data = file_bytes.decode(detected_encoding)
            except UnicodeDecodeError:
                self.log.emit(f"Skipping {csv_file.name} due to decoding error with {detected_encoding}.")
                continue

            io_string = StringIO(decoded_file_data)
            reader = csv.DictReader(io_string, delimiter='\t')

            for row in reader:
                if not all(col in row for col in ['<DATE>', '<BALANCE>', '<EQUITY>']):
                ...
                try:
                    ...
                    self.log.emit(f"Skipping row with invalid data in {csv_file.name}: {row}")
                ...
                all_csv_data.append({
                    ...
                    'balance': balance,
                    'equ': equity,
                    'file': os.path.basename(csv_file.name)
                })
        ...
        if not all_csv_data:
            self.log.emit("No valid equity data found in the uploaded files.")
            self.finished.emit({})
            return

        # Now perform analysis
        mggdd, cntmggdd, lines, groupVal = self.perform_analysis(
            all_csv_data, self.drop_threshold, self.global_dd, self.time_distance
        )

        # Prepare data for display
        # Create DataFrame from all_csv_data
        df_all_data = pd.DataFrame(all_csv_data)
        df_all_data.sort_values('dt', inplace=True)

        # Calculate profit/loss per month
        profit_loss_table = self.calculate_profit_loss_by_month(df_all_data)
        ...
        context = {
            ...
            'profit_loss_table': profit_loss_table
        }

        self.finished.emit(context)

    # Function to calculate maximum equity in a range
    def my_max(self, data, from_idx, to_idx):
        max_value = 0
    ...
    def perform_analysis(self, all_csv_data, drop_threshold=2000, global_dd=3000, time_distance=30):
        mggdd = 0
        cntmggdd = 0
        filtered_data = []
        first = True
        file_b = ""
        file_start_idx = 0
        time_distance_td = timedelta(minutes=time_distance)

        # Calculate drawdowns and filter data
        for i in range(len(all_csv_data)):
            current_data = all_csv_data[i]
            if not first:
                if current_data['file'] != file_b:  # New file
                    file_b = current_data['file']
                    all_csv_data[i]['equ'] = 0
                    file_start_idx = i
                max_equity_so_far = self.my_max(all_csv_data, file_start_idx, i)
                previous_equ = all_csv_data[i - 1]['equ']
                current_equ = current_data['equ']
                if max_equity_so_far == 0:
                    now2 = 0
                else:
                    now2 = (current_equ - max_equity_so_far) / max_equity_so_far * previous_equ

                if now2 < 0 and abs(now2) >= drop_threshold:
                    current_data['vlcalc'] = now2
        ...
        max_group_val = 0
        current_group_val = 0
        added_file_group = set()
        added_date = set()
        to_hide = []
        last_added_i = 0

        for i in range(len(filtered_data)):
            if 'equ' not in filtered_data[i] or filtered_data[i]['equ'] == 0:
                continue
            if i < len(filtered_data) - 1:
                dtDate1 = filtered_data[last_added_i]['dt']
                dtDate2 = filtered_data[i]['dt']
                if dtDate2 - dtDate1 > time_distance_td:
                    current_group_val = 0
                    if max_group_val < abs(current_group_val):
                        max_group_val = abs(current_group_val)
                    group += 1

            if (filtered_data[i]['file'], group) in added_file_group:
                # Check if new vlcalc is higher than the previous one of same file: if so, replace it
                for item in lines:
                    if item['file'] == filtered_data[i]['file'] and item['group'] == group:
                        if abs(item['vlcalc']) < abs(filtered_data[i]['vlcalc']):
                            old_dt = item['dt']
                            current_group_val -= item['vlcalc']
                            item['vlcalc'] = filtered_data[i]['vlcalc']
                            item['dt'] = filtered_data[i]['dt']
                            current_group_val += filtered_data[i]['vlcalc']
                        break
                group += 1
            else:
                added_file_group.add((filtered_data[i]['file'], group))
                current_group_val += filtered_data[i]['vlcalc']
                lines.append({
                    'dt': filtered_data[i]['dt'],
                    'file': filtered_data[i]['file'],
                    'vlcalc': filtered_data[i]['vlcalc'],
                    'group': group,
                    'color': 'rgb(0,0,0)',
                    'value': 0
                })
                last_added_i = i
                data_temp = filtered_data[i]
                added = False
                added_files = set()
                j = i
                while j < len(filtered_data):
                    dtDate1 = filtered_data[last_added_i]['dt']
                    dtDate2 = filtered_data[j]['dt']
                    if dtDate2 - dtDate1 > time_distance_td:
                        break
                    if filtered_data[j]['file'] == data_temp['file']:
                        # Change if higher value
                        for item in lines:
                            if item['file'] == filtered_data[j]['file'] and item['group'] == group:
                                if abs(item['vlcalc']) < abs(filtered_data[j]['vlcalc']):
                                    old_dt = item['dt']
                                    current_group_val -= item['vlcalc']
                                    item['vlcalc'] = filtered_data[j]['vlcalc']
                                    item['dt'] = filtered_data[j]['dt']
                                    current_group_val += filtered_data[j]['vlcalc']
                                break
                        i = j
                        j += 1
                        continue
                    elif (filtered_data[j]['file'], group) not in added_file_group and filtered_data[j]['file'] not in added_files:
                        added_files.add(filtered_data[j]['file'])
                        added = True
                        lines.append({
                            'dt': filtered_data[j]['dt'],
                            'file': filtered_data[j]['file'],
                            'vlcalc': filtered_data[j]['vlcalc'],
                            'group': group,
                            'color': 'rgb(0,0,0)',
                            'value': 0
                        })
                        added_date.add(filtered_data[j]['dt'])
                    else:
                        added = True
                        # Check if new vlcalc is higher than the previous one of same file: if so, replace it
                        for item in lines:
                            if item['file'] == filtered_data[j]['file'] and item['group'] == group:
                                if abs(item['vlcalc']) < abs(filtered_data[j]['vlcalc']):
                                    ...
                                    item['dt'] = filtered_data[j]['dt']
                                    added_date.add(filtered_data[j]['dt'])
                                    added_date.discard(old_dt)
                                break
                    j += 1
                group += 1
                if not added:
                    if filtered_data[i]['dt'] in added_date:
                        to_hide.append((filtered_data[i]['dt'], group))
            if i == 0:
                continue

        for dt, group_num in to_hide:
            lines = [item for item in lines if not (item['dt'] == dt and item['group'] == group_num)]

        # Calculate groupVal
        for line in lines:
            group_num = line['group']
            vlcalc = line['vlcalc']
            if group_num not in groupVal:
                groupVal[group_num] = vlcalc
            else:
                groupVal[group_num] += vlcalc

        # Update mggdd and cntmggdd
        max_group_val = 0
        cntmggdd = 0
        last_group = None
        for line in lines:
            group_num = line['group']
            group_dd = groupVal[group_num]
            if abs(group_dd) > abs(mggdd):
                mggdd = group_dd
            if abs(group_dd) > self.global_dd and last_group != group_num:
                cntmggdd += 1
                last_group = group_num
            if abs(group_dd) > max_group_val:
                max_group_val = abs(group_dd)

        return mggdd, cntmggdd, lines, groupVal

    # Function to calculate profit/loss by month
    def calculate_profit_loss_by_month(self, df_all_data):
        # Create a 'Year-Month' column
        df_all_data['Year_Month'] = df_all_data['dt'].dt.to_period('M')
        df_all_data.sort_values(['dt'], inplace=True)

        # Get the first and last balance for each file and each month
        df_all_data['File_Month'] = df_all_data['file'] + '_' + df_all_data['Year_Month'].astype(str)
        first_balances = df_all_data.groupby('File_Month').first().reset_index()
        last_balances = df_all_data.groupby('File_Month').last().reset_index()

        # Merge first and last balances
        merged_balances = pd.merge(first_balances, last_balances, on='File_Month', suffixes=('_first', '_last'))

        # Calculate profit/loss per file per month
        merged_balances['Monthly_PL'] = merged_balances['balance_last'] - merged_balances['balance_first']
        merged_balances['Year_Month'] = merged_balances['Year_Month_first']

        # Sum profit/loss across all files for each month
        monthly_pl = merged_balances.groupby('Year_Month').agg({'Monthly_PL': 'sum'}).reset_index()

        # Calculate cumulative profit/loss
        monthly_pl['Cumulative_PL'] = monthly_pl['Monthly_PL'].cumsum()

        profit_loss_table = monthly_pl.to_dict('records')
...
class SimpleUploadedFile:
    """
    A simple class to mimic Django's UploadedFile for compatibility with the processing logic.
    """
...
# ---------------------------------
# Function to Extract Symbol from Set File Name
# ---------------------------------
...
# ---------------------------------
# Backtest and Report Generation for full auto
# ---------------------------------
...
def run_full_automation(params, stop_event, progress_var,progress_label, total_set_files, start_button, stop_button):
    ...
    print(params['SET_FILE_LIST'])
    print('%%%%%%%%%%%%')
    print(params['SET_FILES_FOLDER'])
    try:
        # Unpack parameters
        MT5_PATH = params['MT5_PATH']
        MT5_DATA_FOLDER = params['MT5_DATA_FOLDER']
...
def create_gui():
    ...
    def start_process_full_automation():
        ...
        params = {
            ...
            'FROM_DATE_CREATE_OOS': from_date_var_full_automation_create_oos.get(),
            'TO_DATE_CREATE_OOS': to_date_var_full_automation_create_oos.get(),
        ...
        print('&&&&&&&&&&&&&&')
        print(totalEstimatedProfit_checkbox.get())
        print(maxOriginalDd_checkbox.get())
        # Initialize stop event
